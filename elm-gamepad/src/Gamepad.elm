module Gamepad exposing
    ( Blob
    , animationFrameTimestamp, animationFrameDelta
    , UserMappings, emptyUserMappings, userMappingsFromString, userMappingsToString, userMappingsDecoder, encodeUserMappings
    , Gamepad, Analog(..), Digital(..), getGamepads, getIndex, isPressed, wasClicked, wasReleased, value, leftStickPosition, rightStickPosition, dpadPosition
    , RemapModel, RemapMsg, remapInit, remapSubscriptions, remapUpdate, remapView, isRemapping, haveUnmappedGamepads
    )
...
getGamepadMapping (UserMappings database) frame =
    case Dict.get ( frame.index, frame.id ) database.byIndexAndId of
        ...
        Nothing ->
            -- Hum feels like standard is "" in firefox
            if frame.mapping == "standard" || frame.mapping == "" then
